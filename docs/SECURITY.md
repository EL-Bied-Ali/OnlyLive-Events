...
| Password reset | Mitigated, with a known limitation | `lib/auth/passwordReset.ts`: `/api/auth/forgot-password` returns the identical neutral response whether or not the email has an account, a 256-bit random token is generated and only its SHA-256 hash is ever persisted or logged, `/api/auth/reset-password` claims a token via an atomic `updateMany` (`usedAt IS NULL AND expiresAt` in the future) inside the same transaction as the password update itself — so a crash between claiming and applying can never burn a token without actually changing the password, and concurrent submissions of the same link can only ever let one succeed. Tokens expire after 1 hour and are single-use. Both endpoints are rate-limited by IP (and the request endpoint additionally by HMAC'd email). **Known limitation**: because customer sessions are JWTs, not database-backed (see Session fixation row below), a password reset cannot revoke a customer's other already-active sessions — same documented tradeoff as everywhere else this limitation applies, mitigated by the existing session `maxAge`, not solved newly here. |
