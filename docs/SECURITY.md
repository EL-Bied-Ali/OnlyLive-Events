...
| Broken access control / IDOR | Mitigated | Every sensitive route/action calls `requireCustomer()`/`requireAdminRole()` explicitly (never inferred from hidden UI). Catalogue Server Actions allow only admin/super-admin; support is read-only. Admin pages reject scanner accounts; scanner pages/API independently accept only scanner/admin/super-admin and reject customer/support sessions. Order/ticket ownership mismatches return **404**, not 403, so a non-owner can't even confirm the resource exists. |
