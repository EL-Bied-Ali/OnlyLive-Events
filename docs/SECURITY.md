...
| Email enumeration | Partially mitigated | `/api/admin/login` returns the identical `INVALID_CREDENTIALS` error for both "no such admin" and "wrong password". `/api/customers/register` **does** reveal whether an email is already registered (`EMAIL_TAKEN`, 409) — a deliberate, documented tradeoff for this session (registration UX), not an oversight; login itself does not leak this for customers either (next-auth's Credentials `authorize()` returns `null` uniformly on any failure). `/api/auth/forgot-password` also returns the identical neutral response either way (see the Password reset row below) — unlike registration, this flow must not leak account existence. |
| Password reset | Mitigated | `lib/auth/passwordReset.ts`: `/api/auth/forgot-password` returns the identical neutral response whether or not the email has an account — including when a known account's email fails to send (the created token is deleted and the failure swallowed, so a provider outage can't be used to distinguish a known account from an unknown one). A 256-bit random token is generated and only its SHA-256 hash is ever persisted; the raw token lives in the emailed link's URL **fragment** (`#token=...`, never a query string, so it can't reach access/runtime logs or browser history) and the email itself is marked `sensitive`, so `ConsoleEmailProvider` (used on Preview) never prints recipient/subject/body for it. `/api/auth/reset-password` claims a token via an atomic `updateMany` (`usedAt IS NULL AND expiresAt` in the future) inside the same transaction as the password update itself — so a crash between claiming and applying can never burn a token without actually changing the password, and concurrent submissions of the same link can only ever let one succeed. Tokens expire after 1 hour and are single-use. Both endpoints are rate-limited by IP (and the request endpoint additionally by HMAC'd email). A successful reset also increments the user's `authVersion` in that same transaction; see the Session fixation row below — this is what makes the reset revoke the account's other already-active sessions too, closing the limitation this row used to document. |
| Session fixation | Mitigated | Admin session tokens are freshly generated (`crypto.randomBytes(32)`) on every login and stored hashed; customer sessions are Auth.js JWTs signed with `NEXTAUTH_SECRET`, re-issued on sign-in. Customer sessions still have no persisted row to delete server-side on demand (the Credentials provider only supports the JWT strategy), but `User.authVersion` gives one specific, currently-wired revocation path: every JWT carries the `authVersion` it was issued with, the `session()` callback in `lib/auth/customer.ts` compares it against the live column on every session read, and a password reset increments it — so that one event invalidates every session issued before it, not just the browser that performed the reset. There is currently no admin-facing "sign this account out everywhere" action outside of a password reset; that remains a gap if a session is suspected compromised without the customer also wanting to change their password. |
