...
**Real sandbox API-key-scope finding (2026-09-22):** with both fixes above in place, a retried refund returned HTTP `202` (request syntax accepted) but the response *body* itself carried a synchronous business failure: `status: "FAILED"`, `providerRefundId: null`, `failureCode: "BAAS_CHARI_ERROR"`, `failureMessage: "[REFUND_MERCHANT_CARD_PAYMENT] Chari API error (HTTP 403): 403 Forbidden ... Access denied. Missing required scopes: operations:refund"` (correlation id `46965bf5-7d91-4655-9e71-7d909e5a11b0`). This is **not a code bug** — the configured sandbox `CHARIPAY_API_KEY` was provisioned without the `operations:refund` scope, consistent with checklist item 1's least-privilege guidance below having only ever covered read/checkout/webhook operations. `refundId`/`refundReference` are `null` on this failure — nothing was created provider-side to reconcile. **Blocked pending a portal-side fix:** whoever holds ChariPay merchant-portal access must grant the existing sandbox key the `operations:refund` scope (or issue a new key that has it) before a real `refund.succeeded`/`refund.failed` webhook can ever be captured. The code-side gap this exposed has been fixed regardless: a 2xx HTTP response whose body reports a synchronous `failureCode`/`failureMessage` was previously discarded entirely (the thrown error said only `status: "FAILED"`, no indication of why) — both fields are now safely surfaced (bounded code, sandbox-only redacted message) the same way the `!response.ok` path already handles `MISSING_PARAMETER`.
...
8. exercise full and partial refund success/failure; **blocked on the sandbox API key, not the integration** — two real code-side bugs found and fixed via real sandbox attempts (2026-09-18 `operationId` vs `externalId`; 2026-09-22 `reason` closed-enum vs free text — see "Refund lifecycle" above for both), but the next real attempt (2026-09-22, correlation id `46965bf5-7d91-4655-9e71-7d909e5a11b0`) returned HTTP 202 with a synchronous body-level failure: `failureCode: "BAAS_CHARI_ERROR"`, "Access denied. Missing required scopes: operations:refund". The configured sandbox key needs that scope granted (or a new key issued with it) via the ChariPay merchant portal before this item can proceed further. Item remains open until that portal-side grant happens and a real refund is accepted/finalized with its webhook/status payload pinned;
