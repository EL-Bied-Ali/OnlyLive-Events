...
- Because `Chari-Event-Id`/`Chari-Event-Type` are unsigned, one legitimately-signed body can be replayed with many fabricated ids/types inside the ±5 min timestamp window; each fabricated id misses the `existingEvent` dedup and would otherwise trigger a fresh ledger lookup every time. This cannot mutate financial state (order-status guarded), but is rate-limited per Payment (`{ limit: 20, windowMs: 15 min }`, via the same `lib/rateLimit.ts` primitive used elsewhere) so it cannot burn ChariPay API quota unbounded; exceeding it returns `429` before the provider is called, and genuine same-event-id redeliveries never consume this budget.
...
8. exercise full and partial refund success/failure; **in progress** — a real 5.00 MAD partial attempt on 2026-09-18 was definitively rejected as HTTP 400 / `MISSING_PARAMETER` while using the documented `externalId` identifier path; exact required fields were present, so the integration now resolves the already-verified ledger transaction and submits by canonical `operationId` instead. Item remains open until a real refund is accepted/finalized and its webhook/status payload is pinned;
